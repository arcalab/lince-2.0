...
object PlotToTrace:
  ...
  private def addEvent(t: Double, v: String, value: Double)(using res: MutMap[Double,List[Valuation]]): Unit =
    res.get(t) match
      case Some(va::vals) if va.contains(Location(None, v)) =>
        val more = if vals.isEmpty then List(Map(Location(None, v) -> value))
                                   else vals.map(va2 => (va2 + (Location(None, v) -> value)))
        res += (t -> (va :: more)) 
      case Some(vals) =>
        res += (t -> vals.map(va2 => va2 + (Location(None, v) -> value)))
      case None => res += (t -> List(Map(Location(None, v) -> value)))
